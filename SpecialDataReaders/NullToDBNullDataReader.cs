...
namespace Gerk.SpecialDataReaders
{
	internal class NullToDBNullDataReader<T> : IDataReaderWrapper<T> where T : IDataReader
	{
		...
		public object this[string name] => dr[name].NullToDBNull();
		...
		public IDataReader GetData(int i) => new NullToDBNullDataReader(dr.GetData(i));
		...
		public object GetValue(int i) => dr.GetValue(i).NullToDBNull();
		public int GetValues(object[] values)
		{
			var output = dr.GetValues(values);
			for (int i = 0; i < values.Length; i++)
				values[i] = values.NullToDBNull();
			return output;
		}
